...
import { RemoveSession, GetSession } from "/content/github.io/sesame/service/admin/connect/storage.js";
...
export async function OpenLogIn() {
    ...
    if (session.name === '' && session.token === '') {
        ...
        const content = await FetchContent('/content/github.io/utils/uis/sesame/log/login.txt','txt');
        await AppendToButton(content);

        async function LoginAction() {
            document.querySelector('.login-button').addEventListener('click', function() {
                window.location = '/content/github.io/sesame/service/admin/log/index.html';
            });
        }
        ...
        const content = await FetchContent('/content/github.io/utils/uis/sesame/log/logout.txt','txt');
        await AppendToButton(content);
